<template>
  <div :style="cssStyle">
    ...
    <div v-else ref="chartRef" class="widthP100" />
  </div>
</template>
...
let chartConfig: any = {} // 配置参数
let chartOption: any = {} // 配置参数


// 计算属性，用于合并 chartConfig 和 chartOption
...
// 如果改变的是引用类型，比如seriesData是数组，会触发watch监听，但newOptions === oldOptions，
// 如果改变的是基础类型，比如string/number/boolean，则不会触发watch监听。所以加上if (!_.isEqual(newOptions, oldOptions))
watch(() => mergedOptions.value, (newOptions, oldOptions) => {
  // console.log('watch :>> ', newOptions, oldOptions);

  // const { chartConfig: _chartOption, ...extra } = props.options

  chartConfig = props.chartConfig
  chartOption = props.chartOption

  if (!chartConfig.seriesData.length) {
    // console.log('没有数据 :>> ');
    return
  }

  isEmpty.value = !props.chartConfig || !Object.keys(props.chartConfig);
...
const initCharts = () => {
  ...
  if (first) {
    ...
    newOption = _.merge({}, defaultOp, chartOption, {
      series: [{
        data: newSeriesData
      }]
    })
    ...
    newOption = _.merge({}, newOption, chartOption, {
      series: [{
        data: newSeriesData
      }]
    })
...
const showTooltipLoop = () => {
  stopLoop()
  const loopTime = chartConfig.loopTime || 2000;
  ...
  let loopFn: any
  if (chartConfig.type === 'bartransverse') {
    num = length - 1
