<template>
  <a-upload v-model:file-list="fileList" name="file" accept=".fif" :before-upload="beforeUploadFile" :title="t('请上传 fif 文件')">
...
function beforeUploadFile(file: File) {
  ...
  const acceptList = ['fif']
  if (!acceptList.includes(suffix)) {
    message.error(t('请上传脑磁图文件'))
    return
  }

  uploadStore.addUploadingList({
    file: file,
    fileType: 5, // 1：患者数据集   2：DICOM文件  3：NIFTI文件  4：脑电图文件   5：脑磁图文件
    filePath: filePath,
    fileName: fullName,
  })
  // setTimeout必须要加
