<template>
  <div class="card-parent" ref="parentRef">
    <div
      ...
      :style="{
        top: cardTop - parentTop + 'px',
        left: cardLeft - parentLeft + 'px',
        width: cardWidth + 'px',
...
const { x: cardLeft , y: cardTop } = useDraggable(titleRef, {
  // initialValue: { x: cardTop.value, y: cardLeft.value }, // 弹窗的初始位置
  onMove: (position) => {
    if (position.x <=0) {
      cardLeft.value = 0
    }
    ...
    // if (position.x >= window.innerWidth - 600) {
    //   cardLeft.value = window.innerWidth - 600
    // }
    // if (position.y >= window.innerHeight - 770) {
    //   cardTop.value = window.innerHeight - 770
    // }
  }
})
