<template>
  <!-- <div class="card-parent" ref="parentRef"> -->
    <div
      id="card2"
      ref="cardRef"
    ...
    >
      ...
      <div
        ...
        ></div>
        <!-- @mousemove.prevent.stop="e => handleResizeMouseMove(e, corner)"
        @mouseup.prevent.stop="e => handleResizeMouseUp(e, corner)" -->
      ...
      <div class="card-title flex-between" ref="titleRef" :style="{ height: title_height + 'px' }">
        <span>
          全脑区蝴蝶图
          <!-- X:{{ lineX }}/ redMarkLine: {{ redMarkLine }}/ {{ zIndex }} -->
        </span>
        <close-outlined class="card-close" @click="closeCardModal" />
      </div>
...
const { x: cardLeft , y: cardTop } = useDraggable(titleRef, {
  initialValue: { x: margin_num, y: margin_num }, // 弹窗的初始位置
  onMove: (position) => {
    // if (position.x <=0) {
    //   cardLeft.value = 0
    // }
    ...
    //   cardTop.value = 0
    // }
    // if (position.x >= window.innerWidth - initialSize.value.width) {
    //   cardLeft.value = window.innerWidth - initialSize.value.width
    // }
    // if (position.y >= window.innerHeight - initialSize.value.height) {
    //   cardTop.value = window.innerHeight - initialSize.value.height
    // }
  }
})
...
.draggable-card {
  ...
  .card-title {
    ...
    font-weight: 600;
    border-bottom: var(--border); // 白色背景
    // border-bottom: 1px solid #2f2f2f; // 黑色背景
    cursor: move;
  }
