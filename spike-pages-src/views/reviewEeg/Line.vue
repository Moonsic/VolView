...
<template>
  <div :style="{ width: '100%', height: chartHeight + 'px' }">
    ...
    <div ref="myEchartRef" :style="{ width: '100%', height: chartHeight - 40 + 'px' }"></div>
...
let topHeight: number = 94 // 浏览器顶部到图表的距离 12+12+68+12=104
let leftWidth: number = 480 // 浏览器宽度到图表的距离 12+240+4+4+40+12 = 72 + 240 = 312
...
function resetRedChannel() {
  ...
  seriesList.forEach((item: any) => {
    item.itemStyle.color = GOOD_COLOR
  })
  myChart.setOption({
    series: seriesList
  })
...
function setSeriesList(type: 'setChartData' | 'setOverlapRatio') {
  ...
  yData.map((item: any, index: number) => {
    ...
    const obj = {
      ...
      itemStyle: {
        color: type === 'setOverlapRatio' ? oldSeriesList[index].itemStyle.color : GOOD_COLOR, // 如果是重叠或者比例尺按钮，要保持原来的颜色
      },
      lineStyle: {
        width: 1
      },
    ...
    if (params.topTarget) {
      ...
      if (channelName) {
        ...
        if (params.offsetX < CHART_OPTION.grid.left && params.topTarget.style?.textAlign === 'right') {
          ...
          seriesList[seriesIndex].itemStyle.color = seriesList[seriesIndex].itemStyle.color === RED_COLOR ? GOOD_COLOR : RED_COLOR // 改变通道颜色
          myChart.setOption({
            series: seriesList
          })
          // 获取红色通道列表，获取图表信息
...
const modal_height = 244          // 右键弹窗的高度 = title30 + 6*item_height
...
function openMenuModal() {
  ...
  // 如果打开的弹窗与点击时的鼠标点重叠，mac和linux上就会发生框选效果，不知道为什么会这样。
  // 于是想着延时，不让点击事件和弹窗同时出现，于是让弹窗延时出现。
  setTimeout(() => {
...
// 添加标注点成功后刷新标注点
const emit = defineEmits(['successClickType', 'setRedLineValue', 'setStartTime'])
